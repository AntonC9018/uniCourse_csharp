...
7. Сделайте еще один класс, `InputLibraryFilter`,
   ...
   - Сделайте функцию ввода фильтра с консоли.
     Сделайте ввод всех фильтров по очереди и попытка их сохранить в `InputLibraryFilter`,
...
```csharp
bool ok = IsValidPassword(password: "Hello123");
```

**Что скрыто:**

* три различных проверки
* детали того, *как* они выполняются

Вызывающему коду важно только то, что “функция говорит, валидный пароль или нет”.

### 2. Инкапсуляция данных на уровне функции

Локальные данные используются для промежуточных расчётов и полностью скрыты.
Снаружи нельзя их прочитать или изменить.

```csharp
double ComputeAverage(int[] values)
{
    int count = values.Length;     // скрытые данные
    int total = 0;                 // скрытые данные
...
```csharp
double avg = ComputeAverage(values: new[] { 10, 20, 30 });
```
...
Снаружи доступен только конечный результат.

### 3. Инкапсуляция операций на уровне класса

Класс позволяет пользоваться возможностями, **не раскрывая, какие внутренние шаги выполняются**.
...
public class TemperatureConverter
{
    ...
    {
        double offset = SubtractOffset(fahrenheit);   // скрытый шаг
        return ApplyRatio(offset);                    // скрытый шаг
    }

    private double SubtractOffset(double value)
    {
        return value - 32;
    }
...
```csharp
var conv = new TemperatureConverter();
double c = conv.ConvertToCelsius(fahrenheit: 98.6);
```
...
Извне виден только метод `ConvertToCelsius`, всё остальное — деталь реализации.

### 4. Инкапсуляция данных на уровне класса

Класс хранит состояние, к которому нельзя получить прямой доступ,
и предоставляет *контролируемый* доступ через методы.

```csharp
public class BankAccount
{
    private decimal _balance;              // скрытые данные
    private readonly List<string> _log;    // скрытые данные
    ...
    {
        _balance += amount;
        _log.Add($"Deposit: {amount}");
    }

    public IEnumerable<string> GetLog()
    {
        return _log.ToArray(); // инкапсуляция списка (не возвращаем оригинал)
    }
}
```
...
* сам список `_log`
* можно смотреть логи, но нельзя менять их извне
* нельзя напрямую изменить `_balance`

### 5. Инкапсуляция специфики создания объекта (фабрика)

Создание объекта может требовать нескольких шагов, и фабрика **скрывает эти детали**.
...
public class Session
{
    ...
    private Session(Guid id, DateTime createdAt, string userName)
    {
        Id = id;
        CreatedAt = createdAt;
        UserName = userName;
    ...
    public static Session CreateForUser(string userName)
    {
        Guid id = Guid.NewGuid();          // скрыто
        DateTime now = DateTime.UtcNow;    // скрыто
        return new Session(id: id, createdAt: now, userName: userName);
    }
}
```
...
```csharp
Session s = Session.CreateForUser(userName: "anton");
```

**Что скрыто:**

* как создаётся ID
* точный формат времени
* какой конструктор используется
* необходимость передавать 3 параметра

Пользователь передаёт только имя — всё остальное создаётся автоматически.
