...
При компиляции, данный код покажет warning, что код является *unreachable*.
Unreachable означает, что этот код гарантировано никогда не выполнится.
</details>

### 3. Условие с `bool` переменной
```csharp
bool execute = true;
if (execute)
{
...
`if` применим только на выражения типа `bool` (или неявно конвертируемые в `bool`), 
тогда как `int` — это числовой тип.

Данный код не скомпилируется.
</details>

### 6. Несколько инструкций в теле `if`
```csharp
if (false)
{
    Console.WriteLine("A");
...
### 9. Изменение переменной условия внутри `if`
```csharp
bool a = true;
if (a)
{
...
Здесь "Else" не напечатается. То, выполнится ли блок `else`, зависит лишь от условия в `if`.

"After Else" тоже не напечатается. 
`return` полностью прерывает выполнение *всей функции*, 
а не только `if`-а, в котором он был использован.
</details>

### 11. `if`-`else` без фигурных скобок
```csharp
if (true)
    Console.WriteLine("A");
else
...
### 12. Смешанный стиль скобок в `if`-`else`
```csharp
if (true)
    Console.WriteLine("A");
else
...
### 13. Цепочка `else if`
Как обычно записывают данный код, используя цепочку `else`-`if`?
```csharp
if (a)
{
    Console.WriteLine("A");
...
### 14. Early return
Попытайтесь представить данный код как цепочку `else`-`if`, семантически ему идентичную.
