...
### 5. Совпадение имен с `out` параметром
```csharp
int a = 0;
F(out a);
Console.WriteLine(a);
...
### 7. Различный тип `out` параметра, но то же имя
```csharp
A a;
F(out a);
Console.WriteLine(a.f1);
...
### 8. Присваивание `int` переменной типа `struct` через `out`
```csharp
A a;
F(out a);
Console.WriteLine(a.f1);
...
### 9. Неполная инициализация полей `out` параметра типа `struct`
```csharp
A a;
F(out a);
Console.WriteLine(a.f1);
...
### 10. Инициализация всех полей `struct` через `out`
```csharp
A a;
F(out a);
Console.WriteLine(a.f1);
...
### 11. Инициализация всего `struct` значения через `out`
```csharp
A a;
F(out a);
Console.WriteLine(a.f1);
...
### 12. Возвращаемое значение при наличии `out`
```csharp
int a = 0;
F(out a);
Console.WriteLine(a);
...
### 13. Сохранение возвращаемого значения при наличии `out`
```csharp
int a = 0;
int b = 0;
b = F(out a);
...
### 15. Передача поля `struct` через `out`
```csharp
Position p = new()
{
    X = 0,
